...
export class LgTvMqtt {
  ...
  private playStateConfig = JSON.stringify(
    this.createAutoDiscoveryConfig("mdi:play-pause", "play", "Play State")
  );
  private appIdConfig = JSON.stringify(
    this.createAutoDiscoveryConfig("mdi:apps", "app", "Application ID")
  );
  private typeConfig = JSON.stringify(
    this.createAutoDiscoveryConfig("mdi:import", "type", "Discovery Type")
  ); // Do we need these?

  private publicOptionRetain: IClientPublishOptions = { qos: 0, retain: true };
  private publicOptionNotRetain: IClientPublishOptions = {
    qos: 0,
    retain: false,
  };
  ...
  start(message: Message) {
    ...
    try {
      ...
      this.logging.log(
        `Connecting to MQTT server with: ${JSON.stringify(this.mqttConfig)}`
      );
      ...
      this.service
        ...
        .on("response", (message: Message) =>
          this.handleForegroundAppResponse(
            message,
            message.payload as ForegroundAppIdResponse
          )
        );
  ...
  private handleForegroundAppResponse(
    message: Message,
    payload: ForegroundAppIdResponse
  ) {
    if (payload && payload.foregroundAppInfo) {
      if (
        ...
        this.logging.log(
          `Send ForegroundAppInfo update to MQTT: ${JSON.stringify(payload)}`
        );
        ...
        this.client?.publish(
          ...
          this.publicOptionNotRetain
        );
      } else {
        this.logging.log(
          `Ignored ForegroundAppInfo because it's no array, or empty: ${JSON.stringify(
            payload
          )}`
        );
        this.client?.publish(
      ...
      this.logging.log(
        `Ignored ForegroundAppInfo because it contains no info: ${JSON.stringify(
          message
        )}`
      );
      this.client?.publish(
  ...
  private sendAutoDiscovery(message: Message) {
    ...
    try {
      this.client?.publish(
        this.topicAutoDiscoveryPlayState,
        this.playStateConfig,
        this.publicOptionRetain,
        (err) => this.handleError(err, this.topicAutoDiscoveryPlayState)
      );

      this.client?.publish(
        this.topicAutoDiscoveryAppId,
        this.appIdConfig,
        this.publicOptionRetain,
        (err) => this.handleError(err, this.topicAutoDiscoveryAppId)
      );

      this.client?.publish(
        this.topicAutoDiscoveryType,
        this.typeConfig,
        this.publicOptionRetain,
        (err) => this.handleError(err, this.topicAutoDiscoveryType)
      );
    } catch (err) {
      this.logging.log(
        "Failed to publish Home Assistant Auto Discovery configs",
        `${JSON.stringify(err)}`
      );
      this.respond(message, { started: false });
  ...
  private publishInitialState(message: Message) {
    ...
    try {
      ...
      this.client?.publish(
        ...
        JSON.stringify(this.createState("idle", "unknown", "unknown")),
        pubOptions,
        (err) => this.handleError(err, this.topicState)
      );
    } catch (err) {
      this.logging.log(
        "Failed to send initial MQTT state",
        `${JSON.stringify(err)}`
      );
      this.respond(message, { started: false });
  ...
  private publishAvailability(message: Message) {
    ...
    try {
      this.client?.publish(
        ...
        "online",
        this.publicOptionRetain,
        (err) => this.handleError(err, this.topicAvailability)
      );
    } catch (err) {
      this.logging.log(
        "Failed to set availability to online",
        `${JSON.stringify(err)}`
      );
      this.respond(message, { started: false });
  ...
  private handleError(err: Error | undefined, topicName: string) {
    if (err) {
      this.logging.log(
        `An error occurred during publish to ${topicName}`,
        `${JSON.stringify(err)}`
      );
    } else {
